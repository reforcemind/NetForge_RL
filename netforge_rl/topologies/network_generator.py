    ...
    def _generate_procedural(self, rng: random.Random) -> GlobalNetworkState:
        ...
        for i, name in enumerate(subnet_names):
            ...
            for j in range(num_hosts):
                ...
                if rng.random() < 0.15 and name != 'OT_Subnet':
                    host.decoy = rng.choice(['Apache', 'SSHD', 'Tomcat', 'active'])
                else:
                    if name == 'OT_Subnet':
                        ...
                        chosen_os, chosen_services, potential_cves = rng.choice(
                            profiles
                        )
                    ...
                    num_vulns = rng.randint(0, min(2, len(potential_cves)))
                    host.vulnerabilities = rng.sample(potential_cves, num_vulns)
        ...
        if domain_controllers:
            dc = rng.choice(domain_controllers)
