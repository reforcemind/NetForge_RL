...
def _eff(success: bool, deltas, obs=None) -> ActionEffect:
    return ActionEffect(success=success, state_deltas=deltas, observation_data=obs or {})

@pytest.mark.fast
def test_no_collision_passes_red_unchanged() -> None:
    effects = {'red_operator': _eff(True, {'hosts/1.1.1.1/privilege': 'User'}), 'blue_dmz': _eff(True, {'hosts/2.2.2.2/status': 'isolated'})}
...
def test_collision_nullifies_red() -> None:
    effects = {'red_operator': _eff(True, {'hosts/1.1.1.1/privilege': 'Root'}), 'blue_dmz': _eff(True, {'hosts/1.1.1.1/status': 'isolated'})}
    out = resolve_conflicts(effects)
    legacy = ConflictResolutionEngine.resolve(copy.deepcopy(effects))
    assert out['red_operator'].success is False
    assert legacy['red_operator'].success is False
    assert out['red_operator'].observation_data['alert'] == 'TEMPORAL_COLLISION_DEFENSE_SUPREMACY'
    assert out['blue_dmz'].success is True
...
def test_collision_resets_list_deltas_to_empty_list() -> None:
    ...
    class _Cmd:
        ...
        def __init__(self, ip):
            self.target_ip = ip
    effects = {'red_operator': _eff(True, [_Cmd('1.1.1.1')]), 'blue_dmz': _eff(True, {'hosts/1.1.1.1/status': 'isolated'})}
    out = resolve_conflicts(effects)
    assert out['red_operator'].success is False
    assert out['red_operator'].state_deltas == []
...
def test_failed_blue_does_not_defend() -> None:
    effects = {'red_operator': _eff(True, {'hosts/1.1.1.1/privilege': 'Root'}), 'blue_dmz': _eff(False, {'hosts/1.1.1.1/status': 'isolated'})}
    out = resolve_conflicts(effects)
    assert out['red_operator'].success is True

@pytest.mark.fast
def test_already_failed_red_passes_through() -> None:
    effects = {'red_operator': _eff(False, {'hosts/1.1.1.1/privilege': 'Root'}), 'blue_dmz': _eff(True, {'hosts/1.1.1.1/status': 'isolated'})}
    out = resolve_conflicts(effects)
    assert out['red_operator'].success is False
...
def test_none_effect_passes_through() -> None:
    effects = {'red_operator': None, 'blue_dmz': _eff(True, {'hosts/1.1.1.1/status': 'isolated'})}
    out = resolve_conflicts(effects)
...
def test_input_dict_is_not_mutated() -> None:
    ...
    effects = {'red_operator': red_eff, 'blue_dmz': _eff(True, {'hosts/1.1.1.1/status': 'isolated'})}
