"""Parity tests: pure ``resolve_conflicts`` vs legacy ``ConflictResolutionEngine``.

The functional version must produce identical resolved effects (success flag,
state_deltas, observation_data['alert']) while leaving the input dict and
inner ActionEffect objects untouched.
"""
...
def test_failed_blue_does_not_defend() -> None:
    ...
    out = resolve_conflicts(effects)
    assert out['red_operator'].success is True  # blue failed -> no defense
...
def test_already_failed_red_passes_through() -> None:
    ...
    out = resolve_conflicts(effects)
    assert out['red_operator'].success is False
    # No alert should be added since we never evaluated this effect for collision
...
def test_input_dict_is_not_mutated() -> None:
    ...
    # Original effect must still report success and keep its delta.
