import pytest
from netforge_rl.core.functional import apply_state_delta, apply_state_deltas, from_global_state, to_global_state
...
def test_host_privilege_delta_parity(global_state) -> None:
    ip = _first_ip(global_state)
    snap = from_global_state(global_state, agent_ids=AGENTS)
    rebuilt = to_global_state(apply_state_delta(snap, f'hosts/{ip}/privilege', 'Root'))
...
def test_host_compromised_by_delta_parity(global_state) -> None:
    ip = _first_ip(global_state)
    snap = from_global_state(global_state, agent_ids=AGENTS)
    rebuilt = to_global_state(apply_state_delta(snap, f'hosts/{ip}/compromised_by', 'red_operator'))
...
def test_host_edr_active_delta_parity(global_state) -> None:
    ip = _first_ip(global_state)
    snap = from_global_state(global_state, agent_ids=AGENTS)
    rebuilt = to_global_state(apply_state_delta(snap, f'hosts/{ip}/edr_active', True))
...
def test_knowledge_delta_parity(global_state) -> None:
    snap = from_global_state(global_state, agent_ids=AGENTS)
    ip = '10.99.99.99'
    rebuilt = to_global_state(apply_state_delta(snap, f'knowledge/red_operator/{ip}'))
...
def test_apply_state_deltas_dict(global_state) -> None:
    ip = _first_ip(global_state)
    snap = from_global_state(global_state, agent_ids=AGENTS)
    deltas = {f'hosts/{ip}/status': 'isolated', f'hosts/{ip}/privilege': 'Root', f'hosts/{ip}/edr_active': True}
