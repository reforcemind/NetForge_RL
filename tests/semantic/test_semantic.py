"""Tests for the Semantic Bridge wrappers (Phase 8 M1)."""
...
def test_action_menu_contains_team_actions() -> None:
    ...
    # Red-operator-only actions show up alongside the shared red team menu.
...
def test_parser_rejects_unknown_action() -> None:
    ips = ['10.0.0.1']
    # 99 is not a registered action id for any blue agent.
...
def test_end_to_end_text_action_loop(env_sim) -> None:
    ...
    # A legal action menu pair: action 0 (IsolateHost), first real IP.
    ...
    # Confirm the env will accept it without crashing.
