...
def test_run_episode_with_scripted_client(env_sim) -> None:
    target_ip = next((ip for ip in env_sim.global_state.all_hosts if not ip.startswith('169.254.')))
...
def test_leaderboard_roundtrip(tmp_path) -> None:
    ...
    for i in range(3):
        append_result(path, EpisodeResult(model_id='gpt-test', scenario='ransomware', steps=10, rewards={'blue_dmz': 5.0 + i, 'red_operator': -2.0}, invalid_replies={'blue_dmz': 0}, final_compromised=2, final_isolated=3))
    summary = summarize(path)
    assert len(summary) == 1
    assert summary[0]['model_id'] == 'gpt-test'
    assert summary[0]['n_episodes'] == 3
    assert summary[0]['mean_total_reward'] == pytest.approx(sum((5.0 + i - 2.0 for i in range(3))) / 3)