...
def test_adapter_penalises_invalid_responses(env_sim) -> None:
    adapter = LMPolicyAdapter(env_sim, controlled_agent='blue_dmz', seed=42, invalid_penalty=-7.5)
    adapter.queries()
    batch_bad = adapter.step(['nope'])
    assert batch_bad.invalid == 1
    adapter2 = LMPolicyAdapter(env_sim, controlled_agent='blue_dmz', seed=42, invalid_penalty=0.0)
    adapter2.queries()
