...
def test_scenario_blue_rewards(scenario, global_state):
    ...
    effect = ActionEffect(success=True, state_deltas={'hosts/10.0.0.5/status': 'isolated'}, observation_data={})
    r1 = scenario.calculate_reward(agent, global_state, effect)
    assert r1 > 0
    global_state.all_hosts['10.0.0.10'].compromised_by = 'None'
    effect = ActionEffect(success=True, state_deltas={'hosts/10.0.0.10/status': 'isolated'}, observation_data={})
    r2 = scenario.calculate_reward(agent, global_state, effect)
    assert r2 < 0
    effect = ActionEffect(success=True, state_deltas={'hosts/10.0.0.5/status': 'online'}, observation_data={})
...
def test_scenario_red_rewards(scenario, global_state):
    agent = 'red_operator'
    effect = ActionEffect(success=True, state_deltas={'hosts/10.0.0.5/privilege': 'Root'}, observation_data={})
    r = scenario.calculate_reward(agent, global_state, effect)
    assert r >= 5.0
    effect = ActionEffect(success=True, state_deltas={'hosts/10.0.0.5/system_integrity': 'compromised'}, observation_data={})
    r = scenario.calculate_reward(agent, global_state, effect)
    assert r >= 10.0
...
def test_scenario_observation_rewards(scenario, global_state):
    agent = 'red_operator'
    effect = ActionEffect(success=True, state_deltas={}, observation_data={'discovered_hosts': ['10.0.0.20', '10.0.0.30']})
    r = scenario.calculate_reward(agent, global_state, effect)
    assert r > 0
    effect = ActionEffect(success=True, state_deltas={}, observation_data={'scan_results': {'port_80': 'open'}})
    r = scenario.calculate_reward(agent, global_state, effect)
    assert r > 0
