...
def test_cloud_hybrid_red_rewarded_for_secure_breach():
    env = NetForgeRLEnv({'scenario_type': 'cloud_hybrid', 'max_ticks': 5})
    env.reset(seed=0)
    secure = next((h for h in env.global_state.all_hosts.values() if h.subnet_cidr == '10.0.1.0/24'), None)
    if secure is None:
        pytest.skip('topology had no Secure subnet host')
    from netforge_rl.core.action import ActionEffect
    eff = ActionEffect(success=True, state_deltas={f'hosts/{secure.ip}/privilege': 'User'}, observation_data={})
...
def test_cloud_hybrid_terminates_when_secure_fully_owned():
    env = NetForgeRLEnv({'scenario_type': 'cloud_hybrid', 'max_ticks': 5})
    env.reset(seed=0)
    secure = [h for h in env.global_state.all_hosts.values() if h.subnet_cidr == '10.0.1.0/24']
    if not secure:
