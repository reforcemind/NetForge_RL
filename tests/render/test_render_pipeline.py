...
def test_snapshot_color_classifications() -> None:
    ...
    gs.all_hosts['10.0.0.2'].privilege = 'Root'
    gs.all_hosts['10.0.0.3'].status = 'isolated'
    snap = snapshot_from_envstate(from_global_state(gs, AGENTS))
    assert snap.n_nodes == 3
    np.testing.assert_allclose(snap.colors[0], COLOR_SECURE, atol=0.001)
    np.testing.assert_allclose(snap.colors[1], COLOR_COMPROMISED, atol=0.001)
    np.testing.assert_allclose(snap.colors[2], COLOR_ISOLATED, atol=0.001)
