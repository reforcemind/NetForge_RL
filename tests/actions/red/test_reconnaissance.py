import pytest
from netforge_rl.actions.red.reconnaissance import (
    NetworkScan,
...
def test_discover_remote_systems_execution(red_agent):
    ...
    state.register_host(
        Host(ip='192.168.1.10', hostname='WebSrv', subnet_cidr=target_subnet)
    )
    result = action.execute(state)
    assert result.success is True
...
def test_discover_remote_systems_decoys(red_agent):
    ...
    state.register_host(host)
    state.register_host(
        Host(ip='169.254.0.1', hostname='Decoy1', subnet_cidr='169.254.0.0/16')
    )
    result = action.execute(state)
    assert result.success is True
