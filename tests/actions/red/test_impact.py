...
def test_impact_execution(red_agent):
    ...
    state.register_host(Host(ip=target_ip, hostname='Target', subnet_cidr='192.168.1.0/24'))
...
def test_kill_process_execution(red_agent):
    ...
    state.register_host(Host(ip=target_ip, hostname='TargetEDR', subnet_cidr='192.168.1.0/24'))
    result = action.execute(state)
    assert result.success is True
