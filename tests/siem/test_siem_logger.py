...
def test_siem_log_action(siem_logger, global_state):
    ...
    host.privilege = 'Root'
    effect = ActionEffect(success=True, state_deltas={}, observation_data={'exploit': target_ip})
    initial_buffer_size = len(global_state.siem_log_buffer)
    siem_logger.log_action('ExploitEternalBlue', effect, global_state, red_agent, target_ip)
...
def test_siem_buffer_rolling(siem_logger, global_state):
    ...
    assert global_state.siem_log_buffer[-1] == (f'Log_{SIEM_BUFFER_MAX + 9}', '10.0.0.0/24')
