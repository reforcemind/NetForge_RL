...
def _assert_snapshot_matches_legacy(env) -> None:
    ...
    for i, ip in enumerate(sorted_ips):
        ...
        expected_status = STATUS_CODES.index(host.status) if host.status in STATUS_CODES else 0
        assert int(snap.hosts.status[i]) == expected_status, f'status drift @{ip}: snap={snap.hosts.status[i]} legacy={host.status}'
        ...
        assert bool(snap.hosts.is_domain_controller[i]) == bool(host.is_domain_controller)
    for j, agent in enumerate(env.possible_agents):
        ...
        assert int(snap.agent_locked_until[j]) == legacy.agent_locked_until.get(agent, 0)
