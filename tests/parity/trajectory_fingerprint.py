...
Hashes the per-step (reward, term, trunc) stream for a fixed-seed rollout.
Rewards are quantized to 6 decimals to absorb harmless FP jitter; observations
are excluded so the hash isn't coupled to PYTHONHASHSEED.
"""
...
def roll_trajectory(seed=42, max_ticks=50, scenario='ransomware'):
    """Replay a deterministic episode and return its Trajectory record."""
    config = {
