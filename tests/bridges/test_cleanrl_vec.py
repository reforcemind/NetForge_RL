"""CleanRL-style VecEnv shim tests."""
...
def test_action_steers_controlled_agent() -> None:
    """Setting target=7 for the controlled agent should make at least one
    host on the batched state register a Blue defensive write."""
    vec = _vec(num_envs=2)
    vec.reset(seed=0)
    obs, reward, *_ = vec.step(np.full(2, 7, dtype=np.int32))
    # Step succeeds and returns finite rewards.
...
def test_advisory_spaces_present() -> None:
    vec = _vec()
    assert vec.single_action_space == ('discrete', 100)
