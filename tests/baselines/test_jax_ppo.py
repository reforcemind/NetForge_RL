...
import numpy as np
from netforge_rl.baselines.jax_ppo import PPOConfig, forward, gae, init_adam, init_mlp_params, adam_update, ppo_loss, train
...
def test_train_smoke_runs_and_returns_losses() -> None:
    cfg = PPOConfig(total_iters=2, num_steps=4, batch_size=4, n_hosts=100, update_epochs=2, num_minibatches=2)
    out = train(cfg)
...
def test_train_rejects_indivisible_minibatch_config() -> None:
    cfg = PPOConfig(total_iters=1, num_steps=4, batch_size=4, n_hosts=100, update_epochs=1, num_minibatches=3)
