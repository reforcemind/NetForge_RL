...
import numpy as np
import pytest
from netforge_rl.baselines import HeuristicBluePolicy, HeuristicRedPolicy, RandomPolicy, evaluate
...
def test_heuristic_blue_targets_compromised_host(env_sim) -> None:
    target = next((ip for ip in env_sim.global_state.all_hosts if not ip.startswith('169.254.')))
...
def test_evaluate_runs_and_returns_results() -> None:
    results = evaluate(HeuristicBluePolicy(seed=0), scenario='ransomware', episodes=2, max_steps=10)
