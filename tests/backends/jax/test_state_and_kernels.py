...
import numpy as np
from netforge_rl.backends.jax import JaxEnvState, apply_compromised_by_delta, apply_host_privilege_delta, apply_host_status_delta, resolve_conflicts_mask, to_jax, to_numpy
from netforge_rl.core.functional import PRIVILEGE_CODES, STATUS_CODES, apply_state_delta, from_global_state
...
def test_to_jax_to_numpy_round_trip(global_state) -> None:
    ...
    np.testing.assert_array_equal(back.hosts.compromised_by_id, snap.hosts.compromised_by_id)
...
def test_status_kernel_matches_functional_core(global_state) -> None:
    ...
    jax_after = apply_host_status_delta(to_jax(snap), jnp.asarray(idx), jnp.asarray(STATUS_CODES.index('isolated'), dtype=jnp.int8))
    np.testing.assert_array_equal(np.asarray(jax_after.hosts.status), cpu_after.hosts.status)
...
def test_privilege_kernel_matches_functional_core(global_state) -> None:
    ...
    jax_after = apply_host_privilege_delta(to_jax(snap), jnp.asarray(idx), jnp.asarray(PRIVILEGE_CODES.index('Root'), dtype=jnp.int8))
    np.testing.assert_array_equal(np.asarray(jax_after.hosts.privilege), cpu_after.hosts.privilege)
...
def test_compromised_by_kernel_matches_functional_core(global_state) -> None:
    ...
    jax_after = apply_compromised_by_delta(to_jax(snap), jnp.asarray(idx), jnp.asarray(AGENTS.index('red_operator'), dtype=jnp.int8))
    np.testing.assert_array_equal(np.asarray(jax_after.hosts.compromised_by_id), cpu_after.hosts.compromised_by_id)
...
def test_status_kernel_is_jit_safe(global_state) -> None:
    ...
    out = jitted(jstate, jnp.asarray(0), jnp.asarray(STATUS_CODES.index('isolated'), dtype=jnp.int8))
...
def test_resolve_conflicts_mask_vmaps_over_envs() -> None:
    n_envs = 8
    red_targets = jnp.tile(jnp.array([[True, False, False]], dtype=bool)[None, :, :], (n_envs, 1, 1))
    blue_targets = jnp.tile(jnp.array([[True, False, False]], dtype=bool)[None, :, :], (n_envs, 1, 1))
    red_ok = jnp.ones((n_envs, 1), dtype=bool)
    blue_ok = jnp.ones((n_envs, 1), dtype=bool)
    batched = jax.vmap(resolve_conflicts_mask)(red_targets, blue_targets, red_ok, blue_ok)
