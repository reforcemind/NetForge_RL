...
def test_privesc_reward_is_higher_than_compromise(global_state) -> None:
    ...
    assert float(r_privesc[0, 0]) == pytest.approx(float(jnp.tanh(3.0 * 0.01)))
...
def test_honeytoken_traps_red_compromise(global_state) -> None:
    ...
    state, _ = step(
        ...
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_DEPLOY_HONEYTOKEN]],
        ),
    )
    ...
    assert float(rewards[0, 0]) == pytest.approx(float(jnp.tanh(1.0 * 0.01)) - 0.5)
...
def test_impact_promotes_integrity_when_root(global_state) -> None:
    ...
    assert int(state.hosts.system_integrity[0, 8]) == INTEGRITY_CODES.index(
        'compromised'
    )
    assert float(rewards[0, 0]) == pytest.approx(float(jnp.tanh(10.0 * 0.01)))
...
def test_cve_exploit_compromises_when_bit_set(global_state) -> None:
    ...
    state, rewards = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_EXPLOIT_ETERNALBLUE]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert int(state.hosts.privilege[0, 13]) == PRIVILEGE_CODES.index('User')
    assert float(rewards[0, 0]) == pytest.approx(float(jnp.tanh(1.5 * 0.01)))
...
def test_recon_sets_red_knowledge_bit(global_state) -> None:
    ...
    assert float(rewards[0, 0]) == pytest.approx(float(jnp.tanh(0.2 * 0.01)))
...
def test_misinform_brands_decoy_as_apache(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_MISINFORM_APACHE]],
        ),
    )
...
def test_rotate_kerberos_clears_red_credentials(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_ROTATE_KERBEROS]],
        ),
    )
...
def test_kill_process_panics_root_host(global_state):
    ...
    state, rewards = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_KILL_PROCESS]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert int(state.hosts.status[0, 25]) == STATUS_CODES.index('kernel_panic')
    assert float(rewards[0, 0]) == pytest.approx(float(jnp.tanh(5.0 * 0.01)))
...
def test_misinform_tomcat_brands_decoy_as_tomcat(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_MISINFORM_TOMCAT]],
        ),
    )
    assert int(state.hosts.decoy[0, idx]) == DECOY_CODES.index('Tomcat')
...
def test_misinform_sshd_brands_decoy_as_sshd(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_MISINFORM_SSHD]],
        ),
    )
    assert int(state.hosts.decoy[0, idx]) == DECOY_CODES.index('SSHD')
...
def test_restore_from_backup_clears_priv_and_integrity(global_state):
    ...
    assert int(state.hosts.system_integrity[0, 42]) == INTEGRITY_CODES.index(
        'compromised'
    )
    state, _ = step(
        state,
        _act(
            ...
            blue_a=[[True]],
            red_type=[[RED_COMPROMISE]],
            blue_type=[[BLUE_RESTORE_FROM_BACKUP]],
        ),
    )
    assert int(state.hosts.privilege[0, 42]) == PRIVILEGE_CODES.index('None')
    assert int(state.hosts.system_integrity[0, 42]) == INTEGRITY_CODES.index('clean')
...
@pytest.mark.fast
def test_spearphishing_needs_windows(global_state):
    from netforge_rl.core.functional import OS_LINUX, OS_WINDOWS
    ...
    state, _ = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_SPEARPHISHING]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert int(state.hosts.privilege[0, linux_idx]) == PRIVILEGE_CODES.index('None')
    state, _ = step(
        state,
        _act(
            red_t=[[win_idx]],
...
def test_ip_fragmentation_acts_as_generic_compromise(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_IP_FRAGMENTATION]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert int(state.hosts.privilege[0, 17]) == PRIVILEGE_CODES.index('User')
...
def test_network_scan_sets_red_knowledge_bit(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_NETWORK_SCAN]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert bool(state.knowledge_mask[0, 0, 77])
...
def test_discover_remote_systems_sets_red_knowledge_bit(global_state):
    ...
    state, _ = step(
        ...
        _act(
            ...
            red_a=[[True]],
            blue_a=[[False]],
            red_type=[[RED_DISCOVER_REMOTE_SYSTEMS]],
            blue_type=[[BLUE_ISOLATE]],
        ),
    )
    assert bool(state.knowledge_mask[0, 0, 78])
