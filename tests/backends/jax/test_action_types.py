...
from netforge_rl.backends.jax import BatchedActions, VectorEnvSpec, initial_batched_state, make_vector_step, to_jax
from netforge_rl.backends.jax.vector_env import BLUE_CONFIGURE_ACL, BLUE_DEPLOY_DECOY, BLUE_DEPLOY_HONEYTOKEN, BLUE_ISOLATE, BLUE_MISINFORM, BLUE_MONITOR, BLUE_REMOVE, BLUE_RESTORE, BLUE_ROTATE_KERBEROS, BLUE_SAT, EXFIL_PER_HOST, RED_COMPROMISE, RED_DUMP_LSASS, RED_EXFILTRATE, RED_EXPLOIT_BLUEKEEP, RED_EXPLOIT_ETERNALBLUE, RED_IMPACT, RED_JUICY_POTATO, RED_KILL_PROCESS, RED_KINETIC, RED_PASS_THE_HASH, RED_PRIVESC, RED_V4L2, RED_RECON, RED_SHARE_INTEL, SAT_DROP
from netforge_rl.core.functional import CVE_CODES
from netforge_rl.core.functional import DECOY_CODES, INTEGRITY_CODES, PRIVILEGE_CODES, STATUS_CODES, from_global_state
...
def _act(*, red_t, blue_t, red_a, blue_a, red_type, blue_type) -> BatchedActions:
    return BatchedActions(red_target_idx=jnp.asarray(red_t, dtype=jnp.int32), blue_target_idx=jnp.asarray(blue_t, dtype=jnp.int32), red_attempt=jnp.asarray(red_a, dtype=jnp.bool_), blue_attempt=jnp.asarray(blue_a, dtype=jnp.bool_), red_action_type=jnp.asarray(red_type, dtype=jnp.int8), blue_action_type=jnp.asarray(blue_type, dtype=jnp.int8))
...
def test_privesc_promotes_user_to_root(global_state) -> None:
    ...
    state, _ = step(state, _act(red_t=[[0]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.privilege[0, 0]) == PRIVILEGE_CODES.index('User')
    state, _ = step(state, _act(red_t=[[0]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PRIVESC]], blue_type=[[BLUE_ISOLATE]]))
...
def test_privesc_noop_on_uncompromised_host(global_state) -> None:
    ...
    state, _ = step(state, _act(red_t=[[5]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PRIVESC]], blue_type=[[BLUE_ISOLATE]]))
...
def test_restore_wipes_priv_and_ownership(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[3]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.privilege[0, 3]) == PRIVILEGE_CODES.index('User')
    assert int(state.hosts.compromised_by_id[0, 3]) == 0
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[3]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_RESTORE]]))
...
def test_restore_beats_simultaneous_red_compromise(global_state) -> None:
    ...
    state, _ = step(state, _act(red_t=[[7]], blue_t=[[7]], red_a=[[True]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_RESTORE]]))
...
def test_privesc_reward_is_higher_than_compromise(global_state) -> None:
    ...
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[0]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    _, r_privesc = step(state, _act(red_t=[[0]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PRIVESC]], blue_type=[[BLUE_ISOLATE]]))
...
def test_deploy_decoy_flips_decoy_field(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    idx = int(next((i for i in range(100) if int(state.hosts.decoy[0, i]) == DECOY_CODES.index('inactive'))))
    state, rewards = step(state, _act(red_t=[[99]], blue_t=[[idx]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_DEPLOY_DECOY]]))
...
def test_honeytoken_traps_red_compromise(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[12]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_DEPLOY_HONEYTOKEN]]))
    assert bool(state.hosts.contains_honeytokens[0, 12])
    _, rewards = step(state, _act(red_t=[[12]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert float(rewards[0, 0]) == pytest.approx(-4.0)

def _own_to_root(state, step, idx: int):
    state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PRIVESC]], blue_type=[[BLUE_ISOLATE]]))
    return state
...
def test_impact_requires_root(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[4]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_IMPACT]], blue_type=[[BLUE_ISOLATE]]))
...
def test_impact_promotes_integrity_when_root(global_state) -> None:
    ...
    state, rewards = step(state, _act(red_t=[[8]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_IMPACT]], blue_type=[[BLUE_ISOLATE]]))
...
def test_kinetic_super_reward(global_state) -> None:
    ...
    state, rewards = step(state, _act(red_t=[[9]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_KINETIC]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.system_integrity[0, 9]) == INTEGRITY_CODES.index('kinetic_destruction')
    assert float(rewards[0, 0]) == pytest.approx(10000.0)
...
def test_restore_clears_integrity(global_state) -> None:
    ...
    state, _ = step(state, _act(red_t=[[10]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_IMPACT]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.system_integrity[0, 10]) == INTEGRITY_CODES.index('compromised')
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[10]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_RESTORE]]))
...
def test_remove_clears_priv_but_keeps_status(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[11]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.privilege[0, 11]) == PRIVILEGE_CODES.index('User')
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[11]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.status[0, 11]) == STATUS_CODES.index('isolated')
    state, rewards = step(state, _act(red_t=[[99]], blue_t=[[11]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_REMOVE]]))
...
def test_sat_decrements_human_vulnerability(global_state) -> None:
    ...
    state, rewards = step(state, _act(red_t=[[99]], blue_t=[[idx]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_SAT]]))
...
def test_bluekeep_needs_cve_bit_set(global_state) -> None:
    ...
    no_bluekeep = [int(i) for i in range(100) if not bool(state.hosts.vuln_mask[0, i, col]) and int(state.hosts.privilege[0, i]) == 0]
    if not no_bluekeep:
        pytest.skip('topology had no BlueKeep-free host')
    idx = no_bluekeep[0]
    before = int(state.hosts.privilege[0, idx])
    state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_EXPLOIT_BLUEKEEP]], blue_type=[[BLUE_ISOLATE]]))
...
def test_cve_exploit_compromises_when_bit_set(global_state) -> None:
    ...
    state = state.__class__(**{**state.__dict__, 'hosts': state.hosts.__class__(**{**state.hosts.__dict__, 'vuln_mask': new_vm})})
    state, rewards = step(state, _act(red_t=[[13]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_EXPLOIT_ETERNALBLUE]], blue_type=[[BLUE_ISOLATE]]))
...
def test_recon_sets_red_knowledge_bit(global_state) -> None:
    ...
    state, rewards = step(state, _act(red_t=[[23]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_RECON]], blue_type=[[BLUE_ISOLATE]]))
...
def test_monitor_sets_blue_knowledge_bit(global_state) -> None:
    ...
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[42]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_MONITOR]]))
...
def test_intel_reward_only_on_first_sighting(global_state) -> None:
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, r_first = step(state, _act(red_t=[[5]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_RECON]], blue_type=[[BLUE_ISOLATE]]))
    state, r_second = step(state, _act(red_t=[[5]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_RECON]], blue_type=[[BLUE_ISOLATE]]))
...
def test_misinform_brands_decoy_as_apache(global_state):
    ...
    step = make_vector_step(spec)
    idx = int(next((i for i in range(100) if int(state.hosts.decoy[0, i]) == DECOY_CODES.index('inactive'))))
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[idx]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_MISINFORM]]))
...
def test_exfiltrate_requires_root(global_state):
    ...
    state, rewards = step(state, _act(red_t=[[6]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_EXFILTRATE]], blue_type=[[BLUE_ISOLATE]]))
    assert float(state.exfiltrated_bytes[0]) == before
...
def test_exfiltrate_accumulates_when_root(global_state):
    ...
    before = float(state.exfiltrated_bytes[0])
    state, rewards = step(state, _act(red_t=[[14]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_EXFILTRATE]], blue_type=[[BLUE_ISOLATE]]))
...
def test_configure_acl_flips_edr_active(global_state):
    ...
    state, rewards = step(state, _act(red_t=[[99]], blue_t=[[22]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_CONFIGURE_ACL]]))
...
def test_share_intel_broadcasts_red_knowledge(global_state):
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[3, 0]], blue_t=[[99]], red_a=[[True, False]], blue_a=[[False]], red_type=[[RED_RECON, RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    assert bool(state.knowledge_mask[0, 0, 3])
    assert not bool(state.knowledge_mask[0, 1, 3])
    state, _ = step(state, _act(red_t=[[0, 0]], blue_t=[[99]], red_a=[[False, True]], blue_a=[[False]], red_type=[[RED_COMPROMISE, RED_SHARE_INTEL]], blue_type=[[BLUE_ISOLATE]]))
...
def test_lsass_loots_token_when_root(global_state):
    ...
    state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_DUMP_LSASS]], blue_type=[[BLUE_ISOLATE]]))
...
def test_pass_the_hash_compromises_with_token(global_state):
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    src = next((i for i in range(100) if bool(state.hosts.host_tokens[0, i].any())), None)
    if src is None:
        pytest.skip('no host with tokens in this seed')
    state = _own_to_root(state, step, idx=src)
    state, _ = step(state, _act(red_t=[[src]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_DUMP_LSASS]], blue_type=[[BLUE_ISOLATE]]))
    src_tokens = state.hosts.host_tokens[0, src]
    tgt = next((i for i in range(100) if i != src and int(state.hosts.privilege[0, i]) == PRIVILEGE_CODES.index('None') and bool(state.hosts.host_tokens[0, i].any()) and (bool((state.hosts.host_tokens[0, i] & ~src_tokens).any()) is False)), None)
    if tgt is None:
        pytest.skip('no PTH-reachable target on this seed')
    state, _ = step(state, _act(red_t=[[tgt]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PASS_THE_HASH]], blue_type=[[BLUE_ISOLATE]]))
...
def test_pass_the_hash_no_op_without_token(global_state):
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    clean_idx = next((i for i in range(100) if int(state.hosts.privilege[0, i]) == PRIVILEGE_CODES.index('None')))
    state, _ = step(state, _act(red_t=[[clean_idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_PASS_THE_HASH]], blue_type=[[BLUE_ISOLATE]]))
...
def test_rotate_kerberos_clears_red_credentials(global_state):
    ...
    state, _ = step(state, _act(red_t=[[candidates[0]]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_DUMP_LSASS]], blue_type=[[BLUE_ISOLATE]]))
    assert bool(state.agent_credentials[0, 0].any())
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[0]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ROTATE_KERBEROS]]))
...
def test_sat_clamps_at_zero(global_state) -> None:
    ...
    idx = int(state.hosts.human_vulnerability[0].argmax())
    for _ in range(50):
        state, _ = step(state, _act(red_t=[[99]], blue_t=[[idx]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_SAT]]))
...
def test_juicy_potato_needs_windows(global_state):
    ...
    for idx in (win_idx, linux_idx):
        state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    state, _ = step(state, _act(red_t=[[win_idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_JUICY_POTATO]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.privilege[0, win_idx]) == PRIVILEGE_CODES.index('Root')
    state, _ = step(state, _act(red_t=[[linux_idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_JUICY_POTATO]], blue_type=[[BLUE_ISOLATE]]))
...
def test_v4l2_needs_linux(global_state):
    ...
    for idx in (linux_idx, win_idx):
        state, _ = step(state, _act(red_t=[[idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    state, _ = step(state, _act(red_t=[[linux_idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_V4L2]], blue_type=[[BLUE_ISOLATE]]))
    assert int(state.hosts.privilege[0, linux_idx]) == PRIVILEGE_CODES.index('Root')
    state, _ = step(state, _act(red_t=[[win_idx]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_V4L2]], blue_type=[[BLUE_ISOLATE]]))
...
def test_kill_process_panics_root_host(global_state):
    ...
    state, rewards = step(state, _act(red_t=[[25]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_KILL_PROCESS]], blue_type=[[BLUE_ISOLATE]]))
...
def test_audit_no_reward_for_reisolating_already_isolated(global_state):
    ...
    state = _state(global_state, batch=1)
    step = make_vector_step(spec)
    state, _ = step(state, _act(red_t=[[99]], blue_t=[[18]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
    state, rewards = step(state, _act(red_t=[[99]], blue_t=[[18]], red_a=[[False]], blue_a=[[True]], red_type=[[RED_COMPROMISE]], blue_type=[[BLUE_ISOLATE]]))
...
def test_audit_no_reward_for_reimpacting_compromised_host(global_state):
    ...
    state, r_first = step(state, _act(red_t=[[33]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_IMPACT]], blue_type=[[BLUE_ISOLATE]]))
    state, r_second = step(state, _act(red_t=[[33]], blue_t=[[99]], red_a=[[True]], blue_a=[[False]], red_type=[[RED_IMPACT]], blue_type=[[BLUE_ISOLATE]]))
    assert float(r_first[0, 0]) > 0.0
    assert float(r_second[0, 0]) == pytest.approx(0.0)