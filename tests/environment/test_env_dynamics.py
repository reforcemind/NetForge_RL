...
def test_soc_budget_per_agent_not_global(env):
    ...
    for other in ('blue_internal', 'blue_restricted'):
        env.event_queue.append({'completion_tick': 10, 'agent': other, 'action': MagicMockAction(), 'effect': ActionEffect(success=True, state_deltas={}, observation_data={}), 'target_ip': None})
...
def test_soc_per_agent_cap_blocks_third_self_action(env):
    ...
    env.global_state.agent_energy[agent] = 50
    for _ in range(2):
        env.event_queue.append({'completion_tick': 999, 'agent': agent, 'action': MagicMockAction(), 'effect': ActionEffect(success=True, state_deltas={}, observation_data={}), 'target_ip': None})
    initial_energy = env.global_state.agent_energy[agent]
    env.step({agent: 0})
    assert env.global_state.agent_energy[agent] == initial_energy
...
def test_honeytoken_trap_alert(env):
    env.reset(seed=42)
    agent = 'red_operator'
    target_ip = next((ip for ip, h in env.global_state.all_hosts.items() if h.subnet_cidr == '192.168.1.0/24'))
    ...
    expected_effect = ActionEffect(success=True, state_deltas={f'hosts/{target_ip}/privilege': 'User'}, observation_data={'exploit': target_ip})
    with patch.object(ExploitEternalBlue, 'execute', return_value=expected_effect):
        action = ExploitEternalBlue(agent, target_ip)
        env.event_queue.append({'agent': agent, 'action': action, 'completion_tick': 6, 'effect': expected_effect, 'target_ip': target_ip})
        for _ in range(6):
            env.step({})
        all_logs = env.global_state.siem_log_buffer
        honey_alerts = [log[0] for log in all_logs if isinstance(log[0], dict) and log[0].get('signature') == 'HONEYTOKEN_TRIGGERED']
        assert len(honey_alerts) > 0, f'HONEYTOKEN_TRIGGERED missing. Buffer content: {all_logs}'
