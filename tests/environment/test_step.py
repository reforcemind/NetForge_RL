...
def test_env_step_interaction(env_sim_local):
    ...
    actions = {agent: env_sim_local.action_space(agent).sample() for agent in env_sim_local.agents}
...
def test_blue_siem_embedding_update(env_sim_local):
    ...
    env_sim_local.siem_logger._push_to_buffer(fake_log, '192.168.1.0/24', env_sim_local.global_state)
    ...
    assert not np.allclose(dmz_obs['siem_embedding'], 0.0), 'blue_dmz embedding is zero — DMZ signal not propagating'
    for agent in ('red_commander', 'red_operator'):
        if agent in obs:
            assert np.allclose(obs[agent]['siem_embedding'], 0.0), f'Embedding for {agent} is non-zero'