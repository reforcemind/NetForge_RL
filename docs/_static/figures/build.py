...
def card(x: float, y: float, w: float, h: float, fill: str, rot: float, inner: str) -> str:
    cx = x + w / 2
    cy = y + h / 2
    return (
...
def t(x: float, y: float, size: int, weight: str, fill: str, text: str, anchor: str = "middle") -> str:
    return (
...
def chip(x: float, y: float, w: float, h: float, fill: str, rot: float, label: str) -> str:
    cx = x + w / 2
    cy = y + h / 2
    return (
        f'<g transform="rotate({rot:.2f} {cx:.0f} {cy:.0f})">'
...
def arrow(x1: float, y1: float, x2: float, y2: float, title: str, qy: float | None = None) -> str:
    mid = (x1 + x2) / 2
...
def purpose():
    title = "the match"
    parts = [header(title, 1180, 400, "Red attacks  -  Blue defends  -  SIEM in the middle")]
    parts.append(
        card(
            36, 78, 350, 220, RED, -0.08,
    ...
    parts.append(t(590, 378, 15, "800", TITLE, "PettingZoo parallel  ->  or Gymnasium Blue-v4"))
    parts.append(close())
    ...
    write_ex(
        ...
        [
            ...
            e_text(240, 44, 700, 24, "Red attacks - Blue defends - SIEM in the middle", TITLE, 18, "center", 12),
            e_rect(36, 78, 350, 220, RED, -0.01, 21),
            e_text(36, 150, 350, 40, "Red", TITLE, 28, "center", 22),
            e_text(36, 190, 350, 80, "discover, exploit\nescalate, exfil\nstarts blind", INK, 18, "center", 23),
            e_rect(414, 78, 350, 220, TEAL, 0.004, 31),
            e_text(414, 150, 350, 40, "Network", TITLE, 28, "center", 32),
            e_text(414, 190, 350, 80, "hosts, subnets\nfirewalls, decoys\ndurative actions", INK, 18, "center", 33),
            e_rect(792, 78, 350, 220, BLUE, 0.002, 41),
            e_text(792, 150, 350, 40, "Blue", TITLE, 28, "center", 42),
            e_text(792, 190, 350, 80, "isolate, restore\nreads SIEM only\nno oracle", INK, 18, "center", 43),
        ],
        1180,
        400,
    )
...
def workflow():
    ...
    for x, y, w, h, fill, rot, a, b in boxes:
        cx = x + w / 2
        parts.append(
            card(x, y, w, h, fill, rot, t(cx, 118, 16, "800", INK, a) + t(cx, 140, 14, "600", INK, b))
        )
    ...
    parts.append(chip(220, 200, 527, 36, BLUE, -1.5, "exploits and isolates take time  -  logs can be late"))
    parts.append(close())
    ...
    labels = [("reset", "seeded net"), ("observe", "SIEM + mask"), ("act", "Red / Blue"), ("resolve", "conflicts"), ("log", "Sysmon-like")]
    ...
    for i, (x, fill, (a, b), w) in enumerate(zip(xs, fills, labels, ws)):
        ...
        if i:
            els.append(e_arrow(xs[i - 1] + ws[i - 1], 128, x - (xs[i - 1] + ws[i - 1]), 0, 100 + i))
    els += [
        e_rect(220, 200, 527, 36, BLUE, -0.026, 110),
        e_text(220, 202, 527, 32, "exploits and isolates take time - logs can be late", INK, 16, "center", 111),
    ]
...
def why_this():
    ...
    rows = [
        (YEL, "true compromise map", "simulator state", TEAL, "SIEM belief graph", "alerts + inventory"),
        (PINK, "instant telemetry", "every exploit is obvious", GREEN, "delayed / noisy logs", "log_latency, drop, decoys"),
        (ORANGE, "one giant isolate", "kill the network to win", PURPLE, "SLA vs security", "keep hosts online"),
        (RED, "scripted campaign only", "one attacker forever", TEAL, "Red population", "random / heuristic / kill-chain"),
        (PINK, "discrete instant acts", "done in one step", GREEN, "durative actions", "ticks until complete"),
    ]
    ...
    for i, (lc, lt, ls, rc, rt, rs) in enumerate(rows):
        parts.append(
            card(
                36, y, 380, 70, lc, 0.04 if i % 2 == 0 else -0.12,
        ...
        parts.append(
            card(
                544, y, 400, 70, rc, -0.08 if i % 2 == 0 else 0.06,
    ...
    parts.append(t(490, 540, 14, "800", TITLE, "Blue trains on logs   -   Red trains on recon   -   the rest is hidden"))
    parts.append(close())
...
def status():
    ...
    def pill(x: float, y: float, w: float, fill: str, mark: str, name: str, rot: float) -> None:
    ...
    parts.append(
        t(36, 448, 14, "800", TITLE, "entry:  gym.make / pettingzoo.make   -   netforge run / evaluate", "start")
    )
    parts.append(t(36, 478, 14, "800", TITLE, "+ ships in this repo    x train elsewhere, run here", "start"))
    parts.append(close())
    write("status.svg", "".join(parts))
    els = [e_text(28, 12, 280, 32, "in the gym", TITLE, 26, "left", 300)]
    rows_e = [
        (80, "APIs", [(36, 220, "PettingZoo", GREEN), (268, 250, "Gymnasium Blue-v4", GREEN), (530, 200, "AEC wrapper", GREEN)]),
        (172, "World", [(36, 200, "SIEM logs", GREEN), (248, 210, "belief graphs", GREEN), (470, 200, "decoys / OT", GREEN), (682, 190, "HTML replay", TEAL)]),
        (264, "Agents", [(36, 200, "heuristics", GREEN), (248, 220, "kill-chain Red", GREEN), (480, 210, "scripted Blue", TEAL)]),
        (356, "Your code", [(36, 160, "MAPPO", CREAM), (208, 150, "QMIX", CREAM), (370, 190, "GNN trainer", CREAM), (572, 200, "LLM agent", CREAM)]),
    ]
    for y, lab, items in rows_e:
        els.append(e_text(36, y - 28, 200, 24, lab, TITLE, 18, "left", y))
        for x, w, name, fill in items:
            els += [e_rect(x, y, w, 56, fill, 0, y + x), e_text(x, y + 12, w, 32, name, TITLE, 16, "center", y + x + 1)]
...
def arena():
    ...
    for x, fill, name, who, how, use in cols:
        ...
        parts.append(
            card(
                x, 78, 350, 180, fill, 0.12 if fill == YEL else (-0.1 if fill == TEAL else 0.06),
    ...
    parts.append(chip(200, 274, 780, 36, BLUE, -0.7, "mission, SLA, false positives, kinetic  -  not return alone"))
    parts.append(close())
    write("arena.svg", "".join(parts))
    els = [
        e_text(28, 12, 280, 32, "eval splits", TITLE, 28, "left", 400),
        e_text(280, 44, 620, 24, "train / dev / hidden - same metrics", TITLE, 18, "center", 401),
    ]
    for x, fill, name, who, how, use in cols:
        els += [
...
def belief():
    ...
    parts = [header(title, 980, 300, "Blue trains on logs   -   full map is diagnostics")]
    parts.append(
        card(
            36, 78, 420, 160, YEL, -0.12,
    ...
    parts.append(chip(220, 250, 540, 36, TEAL, -1.1, "netforge questions belief-vs-oracle"))
    parts.append(close())
    ...
    write_ex(
        ...
        [
            ...
            e_text(36, 120, 420, 80, "SIEM belief\nwhat a SOC would see", INK, 20, "center", 502),
            e_arrow(456, 158, 78, 0, 503),
            e_rect(534, 78, 410, 160, PINK, 0.002, 504),
            e_text(534, 120, 410, 80, "oracle graph\ntrue compromise, debug only", INK, 20, "center", 505),
        ],
        980,
        300,
    )
